...
class User extends Controller
{
    ...
    public function addbrick(){
        ...
        if(isset($_GET['id'])){
            ...
            if(!empty($checking_listing)){
                 return view('error',array('message'=>'You cannot create brick, Because you have already created your own listing.','title'=>'Add Brick'));
            }else{
            ...
            \Stripe\Stripe::setApiKey(config('app.STRIPE_SECRET'));
                try {
                    ...
                    if (!empty($customer) && !empty($customer["id"])) {
                        ...
                        $insert = $this->common_model->insertdata('user_cards',$user_data);
    ...
    public function userbricks(){
        ...
        $data['retail_categories'] = $this->common_model->selectdata('retail_category',['status'=>1],['name'=>'asc']);
        $data['space_type'] = $this->common_model->selectdata('space_type',['brick_status'=>1],['name'=>'asc']);
        $data['bricks'] = $this->common_model->getlistingdetails(['listing.status'=>1,'listing.type'=>2,'listing.user_id'=>Auth::User()->id],items_count());
    ...
    public function bookings(){
        $data['bookings'] = $this->common_model->getbookingRequests(['listing.status'=>1,'booking_requests.request_to'=>Auth::User()->id]);
        return view('user.bookings',$data);
    }
    public function sent_bookings(){
        $data['bookings'] = $this->common_model->getsentbookingRequests(['listing.status'=>1,'booking_requests.request_from'=>Auth::User()->id]);
        return view('user.sent-bookings',$data);
    }
    ...
    public function partnerdetails(){
        return view('user.partner-details');
    }
    public function paymentcards(){
        $data['cards'] = $this->common_model->selectdata('user_cards',['status'=>1,'user_id'=>Auth::User()->id],['id'=>'DESC']);
        return view('user.payment-cards',$data);
    }
